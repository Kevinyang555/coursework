...
# _thread_startup(struct thread * tp)
#
# This function is called from thread.c to start a new thread. The argument
# is a pointer to the thread context. The thread context is a struct thread
...
# };
#
# The first argument to the thread function is in s[1], the second in s[2],
# and so on. The thread function is in s[0]. The thread function is called
# with the arguments in s[1] to s[8]. The thread function is expected to
# return to _thread_exit when it is done. 

        .global _thread_startup
        .type   _thread_startup, @function
     
_thread_startup:
        # load arguments from s[1] to s[8]
        mv a0, s1
        mv a1, s2
        mv a2, s3
        mv a3, s4
        ...
        # Call the entry function in s[0]
        jalr s0
