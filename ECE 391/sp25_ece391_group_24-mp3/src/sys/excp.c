...
// inputs: unsigned int cause: the cause code of the exception
//         struct trap_frame *tfr: pointer to the trap frame 
// outputs: none
// description:
//     handles exceptions occur in U-mode.
//     - page faults: handle_umode_page_fault().
//     - system calls: handle_syscall().
//     - unhandled exceptions: prints an error message .
//==============================================================================================
void handle_umode_exception(unsigned int cause, struct trap_frame * tfr) {
    
    const char * name = NULL;
    char msgbuf[80];
    uintptr_t fa=csrr_stval();  // faulting address
    ...
    if (name != NULL) {
        ...
        case RISCV_SCAUSE_STORE_PAGE_FAULT:
        case RISCV_SCAUSE_INSTR_PAGE_FAULT:
            if (handle_umode_page_fault(tfr, fa)) {
                return;  //handle page fault
            }
            snprintf(msgbuf, sizeof(msgbuf),"%s at %p for %p in U mode",name,(void*)tfr->sepc, (void*)fa);
            break;
