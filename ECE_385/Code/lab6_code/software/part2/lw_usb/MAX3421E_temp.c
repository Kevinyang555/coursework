...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI 
	reg = reg | 0x02; //set write bit
	Status = XSpi_Transfer(&SpiInstance, &reg, NULL, 1); //write reg + 2
	if (Status != XST_SUCCESS) {
		xil_printf("SPI write command error %d\n", Status);
	}

	//write val via SPI
	Status = XSpi_Transfer(&SpiInstance, &val, NULL, 1); //write val
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, &reg, NULL, 1); //write reg + 2
	if (Status != XST_SUCCESS) {
		xil_printf("SPI write command mulitple error %d\n", Status);
	}

	Status = XSpi_Transfer(&SpiInstance, data, NULL, nbytes); 

	if (Status != XST_SUCCESS) {
		xil_printf("SPI write multiple error %d\n", Status);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE sendBuffer = 0x00;
	X_spi_SetSlaveSelect(&SpiInstance, 1);
	//write reg via SPI
	//read val via SPI
	Status = XSpi_Transfer(&SpiInstance, &reg, NULL, 1); //write reg
	if (Status != XST_SUCCESS) {
		xil_printf("SPI read command error %d\n", Status);
	}

	Status = XSpi_Transfer(&SpiInstance, &sendBuffer, &recvBuffer, 1); //read val
	if (Status != XST_SUCCESS) {
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, &reg, NULL, 1); //write reg
	if (Status != XST_SUCCESS) {
		xil_printf("SPI read command multiple error %d\n", Status);
	}

	for(int i = 0; i < nbytes; i++) {
		Status = XSpi_Transfer(&SpiInstance, &sendBuffer, &data[i], 1); //read val
		if (Status != XST_SUCCESS) {
			xil_printf("SPI read multiple error %d\n", Status);
			break;
		}
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
