...
void SPI_init() {
	...
	if (Status != XST_SUCCESS) {
		xil_printf("SPI config init failed\n");
		return;
	}
...
void MAXreg_wr(BYTE reg, BYTE val) {
    BYTE writeBuffer[2];
    writeBuffer[0] = reg | 0x02;
    writeBuffer[1] = val;
    
	//select MAX3421E
	XSpi_SetSlaveSelect(&SpiInstance, 1);

	Status = XSpi_Transfer(&SpiInstance, writeBuffer, NULL, 2); //write reg + 2
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    BYTE writeBuffer[64];  // Fixed size buffer
    ...
    Status = XSpi_Transfer(&SpiInstance, writeBuffer, NULL, nbytes+1); //write reg + 2
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read val via SPI
	Status = XSpi_Transfer(&SpiInstance, writeBuffer, readBuffer, 2); //write reg
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    BYTE readBuffer[64];   // Fixed size buffer
    BYTE writeBuffer[64];  // Fixed size buffer
    ...
    //read val via SPI
    Status = XSpi_Transfer(&SpiInstance, writeBuffer, readBuffer, nbytes+1); //write reg
    //read return code from SPI peripheral
    //if return code != 0 print an error
    if (Status != XST_SUCCESS) {
        xil_printf("SPI read error %d\n", Status);
