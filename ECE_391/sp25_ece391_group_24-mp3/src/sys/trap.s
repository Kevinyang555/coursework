...
smode_trap_entry_from_umode:
        #==============================================================================================
        # void smode_trap_entry_from_umode(void)
        # inputs: none (hardware sets trap context)
        # outputs: none (jumps to exception/interrupt handlers)
        # description:
        #     Trap entry point for traps originating from U-mode into S-mode. 
        #     Saves all general-purpose registers, CSR state (sepc, sstatus, sinstret, etc.),
        #     establishes a trap frame, then calls smode_trap_entry_from_umode_cont to dispatch.
        #     After handler returns, restores registers and control back to U-mode via sret.
        #==============================================================================================
        sd      t6, T6(sp)
        addi    t6, sp, TFRSZ           #use t6 as temp reg to store address of struct stack_anchor

        ld tp, KTP(t6)                  # load ktp into tp (for current thread)
        ld gp, KGP(t6)                  # load kgp into gp (global pointer for this thread)
        ...
        mv       fp, sp                 # establish proper frame pointer
        csrrw   t6, sscratch, zero      #save ssratch to t6 and set sscratch to zero in preparation for S mode
        sd     t6, SP(sp)               # save the user sp on trap frame
       ...
       #switch kernel stack back to ssratch and user stack back to sp
        csrrw   sp, sscratch, sp
        sret    # done!
